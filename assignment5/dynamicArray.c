...
int _smallerIndexHeap(DynArr *heap, int i, int j, comparator compare)
{
  ...
  /* TODO */
	int test= compare(heap->data[i],heap->data[j]);
	if (test==-1){return i;}
	else if (test==1){return j;}
	return -1;
}
...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
  /* FIXME: Write This */
	...
	while (comp == -1)
		{
		comp = compare(heap->data[cur], heap->data[(cur-1)/2]);
		if(comp == -1)
			{
			swapDynArr(heap, cur, (cur-1)/2);//swapDynArr(DynArr *v, int i, int  j)
			cur = (cur-1)/2;
			}
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
  /* FIXME: Write this */
	...
	if (rightChild < max) 
    {	
		small=_smallerIndexHeap(heap,leftChild,rightChild,compare);
		assert (small !=pos);   
		if (compare(heap->data[pos],heap->data[small])==1)
		{ 
...
void removeMinHeap(DynArr *heap, comparator compare)
{
  /* FIXME: Write this */
	int last = heap->size;
	//assert(last!=0);
	if(last==0){printf("CANNOT REMOVE FROM EMPTY LIST!!!!!");return;}
	putDynArr(heap, 0, getDynArr(heap, last-1));
	removeAtDynArr(heap, last-1);
	_adjustHeap(heap, last-1, 0, compare);
